...
(defn- default-progress-file []
  ...
  (let [filename (str "jolt-hegel-test-progress-"
                      (random-uuid)
                      ".log")]
...
(def progress-file (default-progress-file))
(def ^:dynamic *scenario-timeout-ms* 60000)
...
(defn terminal-timeout-regression []
  ...
  (let [deadline (+ (System/nanoTime)
        ...
        identity-a (child-run! "--progress-identity-child" deadline)
        identity-b (child-run! "--progress-identity-child" deadline)
        timed-out (child-run! "--terminal-timeout-child" deadline)
    ...
    (check "default progress artifacts are distinct across runner invocations"
           identities-ok?)
    (check "timeout child aborts after entering its body without a later sentinel"
           timeout-ok?)
...
(defn- timeout-regression-child! []
  (reset-progress!)
  (println "HEGEL_TIMEOUT_PROGRESS" progress-file)
  (flush)
  (binding [*scenario-timeout-ms* 100]
    (scenario "controlled terminal timeout"
              #(do
                 (progress! "BODY ENTERED")
                 (flush)
                 (deref (promise)))))
...
(defn -main [& args]
  (cond
    (= ["--terminal-timeout-child"] args)
    (timeout-regression-child!)

    (= ["--progress-identity-child"] args)
    (progress-identity-child!)

    :else
    (do
      (reset-progress!)
      (scenario "cross-host exception seam" host-exception-seam)
      (scenario "terminal timeout isolation" terminal-timeout-regression)
