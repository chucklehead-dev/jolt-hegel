...
Expose native setup as `joltc -m hegel.install`. Download libhegel from its
upstream release using source-pinned hashes. Build jolt-hegel shims in the public
tag workflow, publish each with a SHA-256 sidecar, and verify the released assets
by downloading them through the same consumer-visible installer.
