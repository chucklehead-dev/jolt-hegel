...
(defn- children-generators [form path ast config]
  (mapv (fn [index child]
          (ast-generator form (conj path :children index) child config))
        (range)
        (:children ast)))

(defn- map-generator [form path ast config]
...
(defn- ast-generator [form path ast {:keys [default-max-size] :as config}]
  ;; Malli lifts :registry out of :properties into the AST node. v1 does not
  ;; resolve registries or references, so reject even an otherwise supported
  ;; schema carrying one instead of silently discarding it.
  (let [properties (:properties ast)
    ...
    (case type
      ...
      :or
      (do (validate-properties! form path properties #{})
          (g/one-of (children-generators form path ast config)))

      :tuple
      (do (validate-properties! form path properties #{})
          (g/tuple* (children-generators form path ast config)))
...
(defn- normalize-config [form config]
  ...
  (let [maximum (if (contains? config :default-max-size)
                  ...
                  default-max-size)]
    (when-not (uint64-size? maximum)
      (adapter-error ::invalid-config
                     "Malli adapter :default-max-size must be a uint64 integer"
                     form [:config :default-max-size]
                     {:config config :key :default-max-size :value maximum}))
    {:default-max-size maximum}))

(defn generator
  "Build a bounded Hegel generator for a nonrecursive Malli schema.
  ...
  use `:default-max-size`, which defaults to 100.

  Malli references, recursion, regex schemas, predicates, functions, classes,
  transforms, custom generator properties, open maps, and default map entries
  are rejected synchronously instead of being filtered after generation."
  ([schema]
   (generator schema {}))
  ([schema config]
   (let [compiled-schema
         ...
         generated (ast-generator form [] ast config)]
     (g/fmap
      (fn [value]
